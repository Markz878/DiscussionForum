...
public sealed partial class CreateTopic
{
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        if (string.IsNullOrWhiteSpace(_userInfo?.UserName))
        {
    ...
    private async Task SubmitAddTopic()
    {
        try
        {
            ...
            AddTopicResult response = await TopicsService.AddTopic(
                AddTopicModel.Title, 
                AddTopicModel.FirstMessage, 
                AddTopicModel?.Files?.Select(x => new AttachedFileInfo() { Name = x.FileName, FileStream = x.OpenReadStream() }).ToArray()
            );
